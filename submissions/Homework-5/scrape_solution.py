...
def parse_hotellist_page(html,f):
    ...
    for hotel_box in hotel_boxes:
        ...
        f.write(hotel_name.replace(" ",'_')+' ')
        href=hotel_box.find("a", {"target" : "_blank"})['href']
        url="http://www.tripadvisor.com"+href
        time.sleep(2)
        # Request page
        headers = { 'User-Agent' : user_agent }
        response = requests.get(url, headers=headers)
        html = response.text.encode('utf-8')
        nsoup=BeautifulSoup(html)
        tr=nsoup.find("form", {"id" : "REVIEW_FILTER_FORM"})
        tr2=tr.findAll('span',{'class' : 'compositeCount'})
        f.write(tr2[0].find(text=True).strip()+' '+tr2[1].find(text=True).strip()+' '+tr2[2].find(text=True).strip()+' '+tr2[3].find(text=True).strip()+' '+tr2[4].find(text=True).strip()+' ')
        tt=tr.find('div',{'class': 'trip_type'})
        tt2=tt.findAll('div',{'class' : 'value'})
        f.write(tt2[0].find(text=True).strip()+' '+tt2[1].find(text=True).strip()+' '+tt2[2].find(text=True).strip()+' '+tt2[3].find(text=True).strip()+' ')
        rs=tr.find('div', {'id' : 'SUMMARYBOX'})
        rs2=rs.findAll("span", {'class' : 'rate sprite-rating_s rating_s'})
        for x in rs2:
            f.write(str(x.find('img')['alt']).split(' ')[0]+' ')
        #f.write('\n')
...
def scrape_hotels(city, state, datadir='data/'):
    ...
    while(True):
        ...
        city_url = parse_hotellist_page(html,f)
